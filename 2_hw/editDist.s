				...
				jbe end_for_init # i <= word2_len
					...
					cmpb (string2-1)(,%ebx), %dl  # if(word1[i-1] == word2[j-1]): COMPARING BYTE
						jnz else_word1_word2
							movl (oldDist-1)(,%ebx,wordsize), %edx
							movl currDist(,%ebx,wordsize), %edx #curDist[j] = oldDist[j - 1];
						else_word1_word2:
							# curDist[j] = min(min(oldDist[j], curDist[j-1]), oldDist[j-1]) + 1;
							push oldDist(,%ebx,wordsize)
							push (currDist-1)(,%ebx,wordsize)
							call min #min oldDist[j],currDist[j-1]
							push %eax
							push (oldDist-1)(,%ebx,wordsize)
							call min #min result oldDist[j-1]
							incl %eax #min +1
							movl %eax,currDist(,%ebx,wordsize)
				incl %ebx # j++
			end_for_alg_j:
